...
def extract_and_merge_commands(py_file_content: str) -> list:
    ...
    while pos < len(py_file_content):
        # 查找下一个函数调用开始
        match = re.search(
            r"gl\.(DUT\d+)\.(send|CheckCommand)\s*\(", py_file_content[pos:]
        )
        if not match:
            break
        ...
        while current_pos < len(py_file_content) and paren_count > 0:
            ...
            if char in ["'", '"'] and not in_string:
                # 检查是否是三引号
                if (
                    current_pos + 2 < len(py_file_content)
                    and py_file_content[current_pos : current_pos + 3] == char * 3
                ):
                    in_string = True
                ...
                if (
                    in_triple_quote
                    and current_pos + 2 < len(py_file_content)
                    and py_file_content[current_pos : current_pos + 3]
                    == string_char * 3
                ):
                    # 三引号结束
                    in_string = False
...
def extract_send_content(full_call: str) -> str:
    ...
    if params_str.startswith("f'''") or params_str.startswith('f"""'):
        ...
        content = (
            params_str[4:-3]
            if params_str.endswith("'''") or params_str.endswith('"""')
            else params_str[4:]
        )
                    ...
                    if isinstance(item, ast.FunctionDef):
                        ...
                        if item.decorator_list:
                            ...
                            first_decorator_line = min(
                                decorator.lineno for decorator in item.decorator_list
                            )
                        ...
                        functions[func_name] = {
                            ...
                            "first_decorator_line": (
                                first_decorator_line if item.decorator_list else None
                            ),
                        }
        ...
        for match in matches:
            ...
            if cmd_type == "send":
                ...
                if end_pos != -1:
                    ...
                    # 提取详细信息
                    check_info = {"description": content, "full_call": full_call}
...
def extract_command_string(cmd_value: str) -> str:
    ...
    if is_triple_single:
        ...
        if cmd_value.startswith("f'") or cmd_value.startswith('f"'):
            ...
            return (
                cmd_value[2:-1]
                if (cmd_value.endswith("'") or cmd_value.endswith('"'))
                else cmd_value[2:]
            )
                ...
                for cmd in command_lines:
                    if cmd.startswith("dis"):
                        ...
                        if cmd in check_command_dict:
                            ...
                            function_code.append(
                                f"        gl.{device_name}.{check_command}"
                            )
                        else:
                            function_code.append(
                                f"        gl.{device_name}.CheckCommand('',"
                            )
                            function_code.append(
                                f"                             cmd=f'{cmd}'"
                            )
                        ...
                        if (is_send_start == 0 and is_send_end == 0) or (
                            is_send_start == 1 and is_send_end == 1
                        ):
                            is_send_start = 1
                            is_send_end = 0
...
def parse_py_func(file):
    ...
    for line in lines:
        ...
        if (
            ...
            func_name = def_body.split("(", 1)[
                0
            ].strip()  # 拆分到(，取前半部分作为函数名
...
def command_to_func(test_script, new_command, old_command, diff_command_list):
    ...
    for func_name in diff_command_list:
        ...
        func_py_path = os.path.join(
            target_dir, "function.py"
        )  # 假设function.py生成在该目录
...
def main():
    ...
    if len(sys.argv) < 4:
        print("错误：缺少必要参数！")
        print(
            "正确用法：python command_write_back.py <file1路径> <file2路径> <file3路径> [file4路径]"
        )
        print(
            "示例：python command_write_back.py ./test.py ./config_before.txt ./config_after.txt"
        )
