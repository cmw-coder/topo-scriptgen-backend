...
class ScriptGenerationService:
    ...
    async def _execute_copy_and_itc_run(self, task_id: str, script_full_path: str):
        ...
        try:
            ...
            #                     pass
            #     except Exception as e:
            #         self.logger.warning(f"设置目录权限失败 {path}: {e}")

            # set_permissions_recursive(target_dir, 0o777)
            # self._send_message(task_id, "info", f"✓ 目标目录已创建并设置权限: {target_dir}", "processing")
    ...
    async def execute_netconf_script_pipeline(
        ...
        try:
            ...
            # 构造 test_point 描述（从 YANG 文件列表生成）
