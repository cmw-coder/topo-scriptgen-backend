...
class Settings:
    ...
    VERSION: str = "1.0.0"
    DESCRIPTION: str = (
        "FastAPI backend for script generation with topo editor and Claude Code integration"
    )
    ...
    def get_scripts_directory(cls) -> str:
        """获取脚本目录（返回使用正斜杠的字符串路径）"""
        work_dir = Path(cls.get_work_directory())
        return str(work_dir / "scripts").replace("\\", "/")
    ...
    # Deploy 日志文件路径
    DEPLOY_LOG_PATH: str = (
        r"D:\Code\green\AIGC\green_test_script_install\ScriptGenerateDev\static\deploy_log.jsonl"
    )
    ...
    @classmethod
    def convert_ip_to_domain(
        cls, device_list: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        将设备列表中的 IP 地址转换为域名
