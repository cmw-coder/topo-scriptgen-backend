...
class DeployRequest(BaseModel):
    ...
    verisonPath: Optional[str] = Field(
        None, description="版本目录，可选，运行脚本需要的版本目录"
    )
    deviceType: Optional[str] = Field(
        "simware9cen",
        description="设备类型，支持simware9cen、simware9dis、simware7dis，默认simware9cen",
    )
...
class NewDeployRequest(BaseModel):
    ...
    verisonPath: Optional[str] = Field(
        None, description="版本目录（兼容旧接口拼写），可选，运行脚本需要的版本目录"
    )
    versionpath: Optional[str] = Field(
        None, description="版本目录（正确拼写），可选，运行脚本需要的版本目录"
    )
    devicetype: Optional[str] = Field(
        "simware9cen",
        description="设备类型，支持simware9cen、simware9dis、simware7dis，默认simware9cen",
...
class ITCResponse(BaseModel):
    ...
    return_info: Optional[Union[str, Dict[str, Any]]] = Field(
        None, description="返回信息（可能是字符串或字典）"
    )
