...
class PythonAnalysisService:
    ...
    async def find_all_python_files(
            ...
            for py_file in resolved_path.rglob("*.py"):
                try:
                    ...
                    if (
                        ...
                        or "test_example" in py_file.parts
                        or "KE" in path_parts_upper
                    ):
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if checkcommand_match:
                    ...
                    if full_call:
                        # 从完整调用中提取信息
                        extracted_info = self._extract_checkcommand_from_full_call(
                            full_call, line_num
                        )
                        if extracted_info:
                            command_info = CommandLineInfo(
                                ...
                                dut_device=(
                                    f"DUT{dut_device.replace('DUT', '')}"
                                    if dut_device.startswith("DUT")
                                    else dut_device
                                ),
                ...
                clear_buffer_match = re.search(
                    r"gl\.(\w+)\.clear_buffer\s*\(", line, re.IGNORECASE
                )
    ...
    def _extract_checkcommand_parameters(
    ...
    def _extract_checkcommand_from_full_call(
        self, full_call: str, line_num: int
    ) -> Optional[Dict[str, Any]]:
        """从完整的CheckCommand调用中提取信息"""
        try:
            # 提取描述（第一个字符串参数）
            desc_match = re.search(
                r'CheckCommand\s*\(\s*[\'"]([^\'"]+)[\'"]\s*,', full_call
            )
