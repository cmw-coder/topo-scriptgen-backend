...
from app.models.claude import (
    ClaudeCommandRequest, ClaudeCommandResponse, ClaudeLogEntry,
    ClaudeLogQuery, ClaudeCommandType
)
...
class ClaudeService:
    ...
    def _get_command(self, command_type: str, custom_command: Optional[str] = None, parameters: Optional[Dict] = None) -> str:
    ...
    async def execute_command(self, request: ClaudeCommandRequest) -> ClaudeCommandResponse:
        """执行Claude Code命令"""
        ...
        try:
            ...
            # 在后台执行命令
            asyncio.create_task(self._execute_command_background(task_id, command, request))
    ...
    async def _execute_command_background(self, task_id: str, command: str, request: ClaudeCommandRequest):
        """后台执行Claude命令"""
        try:
            # 设置工作目录
            working_dir = None
            if request.working_directory:
                ...
                if not working_dir.exists():
                    await self._update_task_status(task_id, "failed", error=f"工作目录不存在: {working_dir}")
                    return
            ...
            async def read_output(stream, is_error=False):
                while True:
                    ...
                    # 实时发送到WebSocket
                    await self._send_to_websocket(task_id, {
                        "type": "output",
                        "content": line_str,
                        "is_error": is_error,
            ...
            if exit_code == 0:
                await self._update_task_status(task_id, "completed", output=combined_output)
            else:
                error_msg = combined_error or f"命令执行失败，退出码: {exit_code}"
                await self._update_task_status(task_id, "failed", error=error_msg, output=combined_output)

            # 记录日志
            await self._log_execution(task_id, command, exit_code, combined_output, combined_error)

        except Exception as e:
            logger.error(f"执行Claude命令失败: {task_id}, 错误: {str(e)}")
            await self._update_task_status(task_id, "failed", error=f"执行失败: {str(e)}")

    async def _update_task_status(self, task_id: str, status: str, output: Optional[str] = None, error: Optional[str] = None):
        """更新任务状态"""
    ...
    async def _log_execution(self, task_id: str, command: str, exit_code: int, output: str, error: str):
        """记录执行日志"""
        try:
    ...
    async def _save_log_entry(self, log_entry: ClaudeLogEntry):
        ...
        try:
            ...
            # 追加日志
            log_line = json.dumps(log_entry.model_dump(), ensure_ascii=False, default=str) + "\n"
